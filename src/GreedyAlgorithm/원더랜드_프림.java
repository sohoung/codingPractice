...
public class 원더랜드_프림 {
    public static void main(String[] args) {
        ...
        int n = sc.nextInt();
        int k = sc.nextInt();
        ArrayList<ArrayList<eg>> arr = new ArrayList<ArrayList<eg>>();
        for(int i = 0; i <= n; i++) {
            arr.add(new ArrayList<eg>());
        }
        int[] ch = new int[n+1];
        for(int i = 0; i < k; i++) {
            int a = sc.nextInt();
            int b = sc.nextInt();
            int c = sc.nextInt();
            arr.get(a).add(new eg(b,c));  // a라는 정점에서 b라는 정점까지 가는 가중치 값을 c
            arr.get(b).add(new eg(a,c));  // b라는 정점에서 a라는 정점까지 가는 가중치 값을 c
            // 무방향 그래프이기 때문에 a에서 b로도 갈 수 있고 b에서 a로 갈 수 있기 때문에 arr.get(b).add(new EDge(a,c))도 포함시켜야한다.
        }
        int answer = 0;
        PriorityQueue<eg> pq = new PriorityQueue<>();  // Edge 객체를 저장하는 PriorityQueue 객체 생성
        pq.offer(new eg(1,0));  // 처음 시작 정점
        while(!pq.isEmpty()) {
            eg tmp = pq.poll();  // 처음 pq에 들어있는 값을 tmp에 넣어준다.
